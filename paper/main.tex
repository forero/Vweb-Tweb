...
of magnitude in mass ($10^{9}$-$10^{14}$) \hMsun {\bf and use} spatial scales
...
$>10^{12}$\hMsun tend to have their shapes %be
 perpendicular to
...
fact that %that 
the main directions of stronger tidal tension and
velocity shear coincide for halos below $10^{12}$\hMpc but are
perpendicular to each other for halos more massive than this
threshold.  {\bf This clearly separates halos into those that live
in sheets and filaments (lower masses) and nodes of the cosmic web.}
...
as a function of {\bf their} large scale environment. In these situations the environment
definition is {\bf usually} based on quantities easily accesible to observations
such as the number density. With the {\bg advent}%advenment 
of large
...
instability. %Using 
Numerical simulations allow the time description
of such structures together with the formation of the virialized dark
...
hint that strenghtened the expected connection between galactic
properties ant their place in the cosmic web.

{\bf With the aid of} high resolution simulations it has been established that halos of
...
There {\bf are} analytical frameworks, such as tidal torque theory, that
...
cosmic flow patterns in the Local Universe. Assuming the correspondence
between the divergence of the cosmic flow velocity field and the local
...
this perspective it is interesting to look at the expected alignment of
...
to scales of $1$ and $0.5$h$^{-1}$Mpc, respectively. For the T-Web we interpolate first
...
before we consider that  COMPLETAR...
...
\caption{Median of $\cos\theta$ quantifying the shape alignment for
  ...
  eigenvector.\label{fig:shape_alignment}.  SUGIERO PONER AMBAS LEYENDAS EN UN SOLO PANEL, SINO PARECE QUE
IZQUIERDA Y DERECHA FUERAN PARA DOS RESOLUCIONES DISTINTAS.} 
\end{figure*}
...
MOVI EL TITULO DE LA SUBSECCION AQUI, ESTABA DOS PARRAFOS MAS ABAJO, PERO TAMBIEN MODIFIQUE EL TEXTO INICIAL
...
%We also wish to directly compare the results of the two
%web tensor algorithms. 
...
We are mainly interested in the alignment of the web near
halos. Therefore we restrict our analysis to the grid cells that are
occupied by halos. Otherwise, if we decided to perform this kind of analysis
on all the grid cells, the statistics would be dominated by the void
regions, because they dominate in number the fraction of cells in the
simuation.
...
high (low) triaxiality corresponding to prolate (oblate) shapes, circularity ($C/A$)  and halo
inner density (virial mass divided by volume out to the virial radius). We
...
of the halo population above the lower mass limit imposed, the halo masses that dominate the
statistics are close to this lower limit $~10^{11}\hMsun$.
We show the result for the Twev and Vweb calculated using the two available
resolutions, but as can be seen in general we find no significant differences in our results.
...
(e.g. Gao, Springel \& White 2005, Li et al., 2008, Lacerna et al., 2011)
where halos of different ages and equal mass show different clustering
amplitudes, which could also be interpreted as a change in the effective
halo mass according to halo properties.
...
assembly bias scenario of Gao et al., these halos tend to show a clustering
...
%Prolateness follow concentration. In the Tweb
...
Circularidad: C/A (en la figura 1 del paper de  Faltenbacher
lo ponen como spherical o aspherical)

Concentracion: sigma (v_dispersion)/ M^(1/3) H^(1/3) (ignore esta constante porque 
solo estamos viendo el 30% superior he inferior

Triaxialidad: (A^2 - B^2)/(A^2-C^2)

Densidad: (este es propio)  masa virial dividido
el volumen como una esfera de radio R_vir

Spin: (no tengo la ecuacion aqui, pero es la del libro de
Mo, Van de Bosh, White que va como proporcional a |J|/ M^{5/3}
